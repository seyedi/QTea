window.onload=function(){
	var slideshow = document.getElementsByClassName('banner').item(0),
	    back  = document.getElementsByClassName('banner-container').item(0),
		train = slideshow.getElementsByClassName('train').item(0),
		lists = back.getElementsByClassName('btns').item(0).getElementsByTagName('li'),
		currentSlide = 0;

	(go2slide = function (n) {
		if(n>lists.length-1) n=0;
		if(n<0) n=lists.length-1;
		train.style.left=(-870*n)+'px';
